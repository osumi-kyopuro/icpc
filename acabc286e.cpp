...
int main() {
    ...
    rep(i,0,n){
        rep(j,0,n){
            cin>>s[i][j];
            if(s[i][j]=='Y'){
                g2[i].push_back({j,a[j]});
                //g2[j].push_back({i,a[i]});
            }
        }
    }
    pair<ll,ll>inf={LLONG_MAX,LLONG_MAX};
    vector<vector<pair<ll,ll>>>dist(n,vector<pair<ll,ll>>(n));
    rep(i,0,n){
        rep(j,0,n){
            if(i==j){
                dist[i][j]={0,0};
    ...
    rep(i,0,q){
        ...
        if(dist[u][v]==inf){
            ...
            ans[i]={dist[u][v].first,-dist[u][v].second+a[u]};    
        }
    }
