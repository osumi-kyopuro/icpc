...
ll func(ll left,ll right,ll op){
    if(op==1){
        cout<<"? "<<left<<" "<<right<<" "<<1<<" "<<n<<endl;
    }
    else{
...
ll binarySearch(ll ok,ll ng,ll op){
    while(ng-ok>1){
        ll mid=ok+(ng-ok)/2;
        //cout<<mid<<endl;
        if(func(ok,mid-1,op)<(mid-ok)){
            ng=mid;
        }
        else{
            ok=mid;
...
int main() {
    cin>>n;
    ll i=binarySearch(1,n+1,1);
    ll j=binarySearch(1,n+1,2);
    cout<<"! "<<i<<" "<<j<<endl;
}
