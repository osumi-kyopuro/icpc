...
int main() {
    ...
    rep(i,0,q){
        ll t,x;
        cin>>t>>x;
        //auto itr=idxNeg1.end();
        //itr--;
        //cout<<(*itr)<<endl;
        if(t==1){
            if(idxNeg1.lower_bound(x%p)==idxNeg1.end()){
                a[*(idxNeg1.begin())]=x;

                idxNeg1.erase(*(idxNeg1.begin()));    
            }
            else{
                a[*(idxNeg1.lower_bound(x%p))]=x;
                //cout<<*(idxNeg1.lower_bound(x%p))<<endl;
                idxNeg1.erase(*(idxNeg1.lower_bound(x%p)));    
            }
