...
#define print_debug(ARG)(cout<< ARG <<endl)

#else

#define print_debug(ARG)(cout)
...
ll dpf(ll l,ll r){
    ...
    else if(dp[l][r]!=LLONG_MIN){
        return dp[l][r];
    }
    else if(l<r){
        return dp[l][r]=max((ll)(dpf(l+1,r)+((l+1<=p[l]&&p[l]<=r)?a[l]:0ll)),(ll)(dpf(l,r-1)+((l<=p[r]&&p[r]<=r-1)?a[r]:0ll)));
    }
}
