...
int main() {
    ll n,k,sl=-1,sr=-1,p=LLONG_MIN,ans=LLONG_MAX,cnt=0;
    bool flag=true;
    cin>>n>>k;
    V x(n);
    rep(i,0,n){
    ...
    while(sl!=p&&sr!=n-1){
        if(cnt==0){
            sl=(p-k<0?0:p-k);
            sr=(p-k<0?p-1+abs(p-k):p-1);
        }
        else{
            sl++;
            sr++;
        }
        if(x[sr]<0&&x[sl]<0||x[sr]>=0&&x[sl]>=0){
            ans=min((x[sl]<0?abs(x[sl]):x[sr]),ans);
        }
        else{
            ans=min((-x[sl]<x[sr]?2*(-x[sl])+x[sr]:-x[sl]+2*x[sr]),ans);
        }
        //cout<<ans<<endl;
        cnt++;
    }
